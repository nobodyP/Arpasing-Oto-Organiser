...
# This is probably unfinished. I have made some edits since the 6/17 build, but they really just make the code more concise. This one was supposed to allow suffixes but it didn't work. ignore this one
# Made in Python 3.11.3 by NobodyP and Psychic_Digit, 6/22/23
...
npf_ = open("oto_split/oto-no_phonemes_found.txt", "w")
...
#oto_path = input("Type or copy the path of your oto.ini file \n>>> ")
...
for line_number in range(len(oto_con)):
    ...
    # This section takes the current entry of oto_con and isolates only the part that has the phoneme or phoneme combo in it.
    oto_slice = oto_con[line_number][oto_con[line_number].find("=") + 3:oto_con[line_number].find(",")]
    
    for letter in range(len(oto_slice)):
        if oto_slice[letter].isdigit():
            global oto_fix
            oto_fix = oto_slice[:letter]
            letter = 0
            break
        else: oto_fix = oto_slice
    print(oto_fix)
