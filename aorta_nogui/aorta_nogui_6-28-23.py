# "AORTA", an Arpasing OTO Reorderer Tool Application
# This is probably unfinished. The phoneme checking system was totally reworked, so hopefully suffixes and prefixes as well as phonemes in the titles should be ok now
# I had to comment out all the stuff for the "no phonemes found" text file because it just doesn't work anymore after i made changes to the phoneme checking system
# Made in Python 3.11.3 by NobodyP and Psychic_Digit, 6/28/23
# I am not very experienced in coding yet, so if you have any ways to improve it, please let me or NobodyP know
...
# reference tuples for every possible phoneme and phoneme combo. There may be a better way to do this, but i don't know how yet 
...
# this list will contain phonemes that may be present within an entry in the oto.ini file
phon_list = []

# to check whether or not a phoneme or phoneme combo has definitely been found
combo_found = False

# opens output files
v_ = open("oto_split/oto-v.txt", "w")
c_ = open("oto_split/oto-c.txt", "w")
vc_ = open("oto_split/oto-vc.txt", "w")
cv_ = open("oto_split/oto-cv.txt", "w")
vv_ = open("oto_split/oto-vv.txt", "w")
cc_ = open("oto_split/oto-cc.txt", "w")
#npf_ = open("oto_split/oto-_no_phonemes_found.txt", "w")


# this function goes through one of the phoneme tuples, and checks if the oto_fix string contains any of the combos. if it does, the phoneme combo gets added to "phon_list", where it will be sorted so the actual definite phoneme can be found 
def check_possiblephonemes(phonemetuple, appendlist, inputstring):
    for index in range(len(phonemetuple)):
        if phonemetuple[index] in inputstring:
            appendlist.append(phonemetuple[index])
            index = 0

# this function is similar to the other one, but it's for after "phon_list" is sorted. it takes the longest combo from "phon_list" (which should be the index of 0 within the list), and checks if it is EQUAL to any of the phonemes within the specified phoneme tuple. if it is, the current entry of the oto.ini file is added to the specified list
def check_final(phonemetuple, appendlist, appenditem, inputstring):
    for index in range(len(phonemetuple)):
        if phonemetuple[index] == inputstring:
...
print("AORTA : Arpasing Oto Reorderer Tool Application")
print("Made by Psychic_Digit and NobodyP")
print("6/28/23 Python 3.11.3\n")
input("Make sure you have a oto.ini file\nin the directory of this program, as well as\na folder named \"oto_split\". If these are\nnot present, the program will not work.\nPress Enter >>> ")

# I want to add a system where you can type in the specific path of the oto.ini file so it doesn't have to be in the same directory as this program, same with the oto_split folder, but I'll figure that out later
oto_con = open("oto.ini", "r").readlines()



# This loop goes through every entry in the oto_con list, and then checks to see if it has an phoneme or phoneme combo in it (V, C, VV, CC, VC, CV). If there is a phoneme in the entry, depending on which phoneme or combo it is, it will be added to one of the lists. There's probably a better, more concise way to do this, but I do not know another way
for line_number in range(len(oto_con)):
    
    phon_list.clear()
    combo_found = False

    # This section takes the current entry of oto_con and isolates the part between the equal sign and the first comma, the part that contains the phoneme or combo
    oto_fix = "".join((z for z in oto_con[line_number][oto_con[line_number].find("="):oto_con[line_number].find(",")] if not z.isdigit()))
    print(oto_fix)

    check_possiblephonemes(VV_TUPLE, phon_list, oto_fix)
    check_possiblephonemes(CC_TUPLE, phon_list, oto_fix)
    check_possiblephonemes(VC_TUPLE, phon_list, oto_fix)
    check_possiblephonemes(CV_TUPLE, phon_list, oto_fix)
    check_possiblephonemes(V_TUPLE, phon_list, oto_fix)
    check_possiblephonemes(C_TUPLE, phon_list, oto_fix)


    phon_list.sort(key=len, reverse=True)
    print(phon_list)


    check_final(VV_TUPLE, VV, oto_con[line_number], phon_list[0])
    if combo_found == True: continue
    check_final(CC_TUPLE, CC, oto_con[line_number], phon_list[0])
    if combo_found == True: continue
    check_final(VC_TUPLE, VC, oto_con[line_number], phon_list[0])
    if combo_found == True: continue
    check_final(CV_TUPLE, CV, oto_con[line_number], phon_list[0])
    if combo_found == True: continue
    check_final(V_TUPLE, V, oto_con[line_number], phon_list[0])
    if combo_found == True: continue
    check_final(C_TUPLE, C, oto_con[line_number], phon_list[0])
    #if combo_found == True: continue
    #else: NPF.append(oto_con[line_number]) # this was for situations where no phonemes were found within the current entry, i don't think it works anymore



############################################################## --- end of loop, writing entries to output files
