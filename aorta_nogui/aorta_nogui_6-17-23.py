# Arpasing oto.ini reorderer/phoneme sorting thing. I call it "AORTA" for "Arpasing OTO Reorderer Tool Application"
# This is unfinished, there are a couple things I want to add that I think would be really good improvements to this program, but for now it works well enough
# Made in Python 3.11.3 by NobodyP and Psychic_Digit, 6/17/23
# I (Psychic_Digit) am not very experienced in coding yet, so if you look at this code and have any ways to improve it, please improve it or let me or NobodyP know! This code is probably really sloppy, but this is the best I can do right now
...
npf_ = open("oto_split/no_phonemes_found.txt", "w")
...
for line_number in range(len(oto_con)):
    combo_found = False
    v_phoneme_found = False
    c_phoneme_found = False



    # This section takes the current entry of oto_con and isolates only the part that has the phoneme or phoneme combo in it. I could definitely make this a bit more concise but I'm not gonna do it right now
    equ_index = oto_con[line_number].find("=")
    oto_equ = oto_con[line_number][equ_index + 3:]

    comma_index = oto_equ.find(",")
    oto_equ_com = oto_equ[:comma_index]

    oto_fix = "".join((z for z in oto_equ_com if not z.isdigit()))
    print(oto_fix)



    for vv_index in range(len(VV_TUPLE)):
        if VV_TUPLE[vv_index] == oto_fix:
            VV.append(oto_con[line_number])
            vv_index = 0
            combo_found = True
            #print("VV combo found in " + "line " + str(line_number) + ": " + str(oto_con[line_number]))
    if combo_found == True: continue


    for cc_index in range(len(CC_TUPLE)):
        if CC_TUPLE[cc_index] == oto_fix:
            CC.append(oto_con[line_number])
            cc_index = 0
            combo_found = True
            #print("CC combo found in " + "line " + str(line_number) + ": " + str(oto_con[line_number]))
    if combo_found == True: continue


    for vc_index in range(len(VC_TUPLE)):
        if VC_TUPLE[vc_index] == oto_fix:
            VC.append(oto_con[line_number])
            vc_index = 0
            combo_found = True
            #print("VC combo found in " + "line " + str(line_number) + ": " + str(oto_con[line_number]))
    if combo_found == True: continue


    for cv_index in range(len(CV_TUPLE)):
        if CV_TUPLE[cv_index] == oto_fix:
            CV.append(oto_con[line_number])
            cv_index = 0
            combo_found = True
            #print("CV combo found in " + "line " + str(line_number) + ": " + str(oto_con[line_number]))
    if combo_found == True: continue



    elif combo_found == False: # this had a purpose before I changed the way the code checks for phonemes, but I don't think it's needed anymore. I'm keeping it for now, because it might cause errors if it's removed



        for v_index in range(len(V_TUPLE)):
            if V_TUPLE[v_index] == oto_fix:
                V.append(oto_con[line_number])
                v_index = 0
                v_phoneme_found = True
                #print("V phoneme found in " + "line " + str(line_number) + ": " + str(oto_con[line_number]))
        if v_phoneme_found == True: continue


        for c_index in range(len(C_TUPLE)):
            if C_TUPLE[c_index] == oto_fix:
                C.append(oto_con[line_number])
                c_index = 0
                c_phoneme_found = True
                #print("C phoneme found in " + "line " + str(line_number) + ": " + str(oto_con[line_number]))
        
        if c_phoneme_found == True: continue
        else: NPF.append(oto_con[line_number])



# writing the values from each of the lists to their respective oto_split text files. the npf_ file is for entries in which no phonemes were found, either because there were none, or because there was an error in the program.
v_.writelines(V)
c_.writelines(C)
vc_.writelines(VC)
cv_.writelines(CV)
