...
# I think I figured out a way to make "no phonemes found" work? not too sure
# Made in Python 3.11.3 by NobodyP and Psychic_Digit, 6/29/23
# I am not very experienced in coding yet, so if you have any ways to improve the code, please let me or NobodyP know
...
for line_number in range(len(oto_con)):
    ...
    if phon_list == []:
        NPF.append(oto_con[line_number])
        print("No possible phonemes found in entry " + str(line_number))
        continue
    else:
        phon_list.sort(key=len, reverse=True)
        print("Phoneme found in entry " + str(line_number) + " : " + phon_list[0] + " " + str(phon_list))
